...
from .basic import Ellipsoid, MultiEllipsoid


class UnitCubeEllipsoidMixture():
    """Overlap of the unit cube and, in some dimensions, an ellipsoid.

    This bound only defines ellipsoids along those dimensions where an
    ellipsoid would have a smaller volume than the unit range.
...
class UnitCubeMultiEllipsoidMixture():
    r"""Overlap of the unit cube and, in some dimensions, an ellipsoid union.
    ...
    n_dim : int
        Number of dimensions.
    dim_cube : numpy.ndarray
        Dimensions along which the bound is not defined via an ellipsoid union.
    dim_ellipsoid : numpy.ndarray
        Dimensions along which the bound is defined via an ellipsoid union.
    multi_ellipsoid : MultiEllipsoid
        Ellipsoid defining the boundary along certain dimensions.
    n_sample : int
        Number of points sampled.
    n_reject : int
    ...
    def compute(cls, points, enlarge_per_dim=1.1, random_state=None):
        ...
        Returns
        -------
        bound : UnitCubeMultiEllipsoidMixture
            The bound.

        """
        bound = cls()

        unit_cube_ellipsoid_mixture = UnitCubeEllipsoidMixture.compute(
            points, enlarge_per_dim=enlarge_per_dim, random_state=random_state)

        for key in ['n_dim', 'dim_cube', 'dim_ellipsoid', 'points_sample',
                    'n_sample', 'n_reject', 'random_state']:
            setattr(bound, key, getattr(unit_cube_ellipsoid_mixture, key))

        bound.multi_ellipsoid = MultiEllipsoid.compute(
            points[:, bound.dim_ellipsoid], enlarge_per_dim=enlarge_per_dim,
            random_state=random_state)
    ...
    def split_ellipsoid(self, allow_overlap=True):
        ...
        """
        return self.multi_ellipsoid.split_ellipsoid(
    ...
    def contains(self, points):
        ...
        """
        return (np.all((points >= 0) & (points < 1), axis=-1) &
                (self.multi_ellipsoid.contains(points[:, self.dim_ellipsoid])))
    ...
    def sample(self, n_points=100):
        ...
        while len(self.points_sample) < n_points:
            ...
            points[:, self.dim_cube] = self.random_state.random(
                size=(n_points, len(self.dim_cube)))
            points[:, self.dim_ellipsoid] = self.multi_ellipsoid.sample(
                n_points)
            points = points[np.all((points >= 0) & (points < 1), axis=-1)]
            self.points_sample = np.vstack([self.points_sample, points])
    ...
    def volume(self):
        ...
        return (self.multi_ellipsoid.volume() + np.log(
            1.0 - self.n_reject / self.n_sample))
